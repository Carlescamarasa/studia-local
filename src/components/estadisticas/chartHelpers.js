 ...
 * @param {Array} rawData - Array de objetos con { inicioISO, ... metricas }
 * @param {string|Date} start - Fecha inicio
 * @param {string|Date} end - Fecha fin
 * @returns {Array} Array de objetos { fecha: 'YYYY-MM-DD', ...metricas }
 */
export function buildDailySeries(rawData, start, end) {
    if (!rawData || rawData.length === 0) return [];

    let dStart = typeof start === 'string' ? parseLocalDate(start) : start;
    let dEnd = typeof end === 'string' ? parseLocalDate(end) : end;
    ...
    while (current <= dEnd && safety < 1825) {
        ...
        if (entry.valoraciones && entry.valoraciones.length > 0) {
            const sum = entry.valoraciones.reduce((a, b) => a + b, 0);
 ...
 * Reglas:
 * <= 45: 'dia'
 * 46-120: 'dia_compacto' (renderiza todos los puntos pero UI debe limitar ticks)
 * 121-210: 'semana'
 * 211-400: 'quincena'
 * > 400: 'mes'
 */
export function chooseBucket(start, end) {
 ...
 */
export function aggregateData(dailySeries, bucketMode) {
    ...
    return Object.values(groups).map(g => {
        ...
        const totalTiempo = g.items.reduce((sum, i) => sum + i.tiempo, 0);
        const totalSesiones = g.items.reduce((sum, i) => sum + i.sesiones, 0);
        const totalCompletados = g.items.reduce((sum, i) => sum + i.completados, 0);
        const totalOmitidos = g.items.reduce((sum, i) => sum + i.omitidos, 0);
        ...
        const diasConValor = g.items.filter(i => i.satisfaccion !== null);
        let avgSat = null;
        if (diasConValor.length > 0) {
            avgSat = diasConValor.reduce((sum, i) => sum + i.satisfaccion, 0) / diasConValor.length;
        }
