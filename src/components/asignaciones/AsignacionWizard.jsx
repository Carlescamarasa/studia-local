...
import { 
  Users, Music, BookOpen, Calendar, Settings, Target, 
  ChevronRight, ChevronLeft, Save, Check, Loader2, AlertCircle 
} from "lucide-react";
...
export default function AsignacionWizard() {
  ...
  // Mutation para crear asignaciones
  const crearAsignacionesMutation = useMutation({
    mutationFn: async (data) => {
      // Verificar error de autenticación en el contexto
      if (authErrorContext) {
      ...
      for (const asignacion of asignaciones) {
        try {
          ...
          console.error('[AsignacionWizard] Error al crear asignación:', {
          ...
          const alumno = usuarios.find(e => e.id === asignacion.alumnoId && (e.rolPersonalizado === 'ESTU' || e.role === 'ESTU'));
    ...
    onSuccess: (results) => {
      ...
      if (formData.adaptarPlanAhora && results.length === 1) {
        ...
        // En modo debug, solo mostrar mensaje (no hay onClose)
      }
    },
    onError: (error) => {
  ...
  useEffect(() => {
    if (formData.fechaSeleccionada) {
      const date = parseLocalDate(formData.fechaSeleccionada);
      const lunes = formatLocalDate(startOfMonday(date));
      setFormData(prev => {
  ...
  return (
    <div className="bg-[var(--color-surface-elevated)] w-full max-w-3xl shadow-card rounded-[var(--radius-modal)] flex flex-col">
      {/* Header */}
