          ...
          <div className="mt-3 space-y-2" onClick={(e) => e.stopPropagation()}>
            {semana.sesiones.map((sesion, sesionIndex) => (
              <SortableSesion
                ...
                toggleEjercicios={toggleEjercicios}
...
export default function PlanEditor({ plan, onClose }: { plan: Plan | null; onClose: () => void }) {
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeId.startsWith('semana-') && overId.startsWith('semana-')) {
      ...
      setFormData({
        ...formData,
        semanas: arrayMove(formData.semanas, oldIndex, newIndex),
      });
      return;
    }
        ...
        >
          <div className="border-b border-[var(--color-border-default)] bg-[var(--color-surface-muted)] rounded-t-[var(--radius-modal)] px-6 py-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="icon" onClick={onClose} className="text-[var(--color-text-primary)] hover:bg-[var(--color-surface)] h-11 w-11 sm:h-9 sm:w-9 min-h-[44px] min-w-[44px] sm:min-h-0 sm:min-w-0 rounded-[var(--btn-radius)] touch-manipulation" aria-label="Cerrar editor">
                <X className="w-5 h-5" />
              </Button>
            </div>
          </div>
